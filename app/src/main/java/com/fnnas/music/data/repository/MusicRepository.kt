...
import com.fnnas.music.network.NasDirectory
import com.fnnas.music.network.NasFileClient
import com.fnnas.music.network.RemoteItem
import com.fnnas.music.network.WebDavHttpException
...
class MusicRepository(
    ...
    suspend fun scanLibrary(onProgress: (ScanProgress) -> Unit): WebDavResult {
        val server = database.nasDao().getActiveNas() ?: return WebDavResult.Failure("先连接你的飞牛 NAS")
        val credentials = currentCredentials() ?: return WebDavResult.Failure("先连接你的飞牛 NAS")
    ...
    private fun validateConnectionForm(form: NasForm, allowStoredPassword: Boolean): String? = when {
        ...
        form.username.isBlank() || (form.password.isBlank() && !allowStoredPassword) -> "请填写飞牛 NAS 用户名和密码"
        else -> null
    }
